...
export default function PillarKinetiCarePage() {
  ...
  async function runAnalysis() {
    ...
    try {
      const durationSec = Math.max(1, (Date.now() - startTime.current) / 1000);
      const res = await fetch("/api/run-check", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({
          journalText: textEntry || "Neuromotor sequence check.",
          baselineMap: 90,
          useCamera: false,
          simulateSpoof: false,
          dwellTimesMs: dwellTimes.current.length > 5 ? dwellTimes.current : [100, 110, 105, 120, 95, 102, 115, 98],
          flightTimesMs: flightTimes.current.length > 5 ? flightTimes.current : [200, 210, 205, 220, 195, 202, 215, 198],
          imuAccelMagnitude: imuAccel.current.length > 10 ? imuAccel.current : new Array(64).fill(9.8),
          kineticareSamplingRateHz: Math.max(10, imuAccel.current.length / durationSec),
        }),
      });
          ...
          <div style={{ display: "flex", flexDirection: "column", gap: 24 }}>
             <div className="glass-card" style={{ padding: 32 }}>
                ...
                <div style={{ background: "#f8fafc", padding: "16px 20px", borderRadius: 8, border: "1px solid var(--border-subtle)", marginBottom: 24 }}>
                   <div className="label" style={{ marginBottom: 12 }}>Typing Sequence</div>
                   <textarea
                     value={textEntry}
                     ...
                     placeholder={sessionState === "running" ? "Type freely here to record latency..." : "Click Start below to enable the sensor."}
                     style={{ width: "100%", height: "100px", padding: "12px", borderRadius: "8px", border: "1px solid #cbd5e1", resize: "none" }}
                   />
                </div>
                ...
                {sessionState === "idle" ? (
                  <button
                    ...
                    style={{ width: "100%", padding: "16px 20px", marginBottom: 12 }}
